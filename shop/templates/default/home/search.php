...
?>/css/csshover.htc);
   ...
   <div class="wrapper nch-breadcrumb-layout">
    <?php if(!empty($output['nav_here'])&&is_array($output['nav_here'])) {?>
    <div class="nch-breadcrumb">
    <?php foreach ($output['nav_here'] as $k=>$value) {?>
    <span class="arrow">&gt;</span>
     <span class="sort-box"> <a href="<?php echo urlShop('search', 'index', array('cate_id' => $value['curr_cate_id']));?>" class="current"><?php echo $value['curr_cate_name'];?><i class="drop-arrow"></i></a> 

      <div class="sort-sub">
       <?php if (!empty($value['cate_array'])&&is_array($value['cate_array'])) {?>
        <ul>
        <?php foreach ($value['cate_array'] as $vc) { ?>
  		<?php if($vc['gc_id']!=$value['curr_cate_id']){?>
        <li><a href="<?php echo urlShop('search', 'index', array('cate_id' => $vc['gc_id']));?>"><?php echo $vc['gc_name'];?></a></li>
        <?php }?>
  		<?php }?>
        </ul>
        <?php }?>
        ...
        <div class="nch-module-filter">	  
            ...
            <dd class="list">
              ...
              <?php if($output['flag'] == 0):?>
                <li><a href="<?php echo replaceParam(array('areaid_1' => 0));?>">全国</a> </li>
                <?php foreach ($output['province'] as $key => $value) { ?>
                  <li><a href="<?php echo replaceParam(array('areaid_1' => $value[0]));?> "> <?php echo $value[1];?></a>  </li>
                <?php }?>
              <?php endif;?>
              <?php if($output['flag'] != 0):?>
	      <li><a href="<?php echo replaceParam(array('areaid_1' => 0));?>">不限</a> </li>
